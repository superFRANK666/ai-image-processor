...
class ModelLoadWorker(QThread):
    ...
    def __init__(self, model_name: str, load_func: Callable):
        ...
        self.result_model: Optional[Any] = None
...
class ModelManager(QObject):
    ...
    def load_model_async(self, model_name: str):
        ...
        with self._state_lock:
            ...
            load_func = self._load_functions[model_name]

        self.loading_started.emit(model_name)

        worker = ModelLoadWorker(model_name, load_func)
    ...
    def load_model_sync(self, model_name: str) -> Optional[Any]:
        ...
        with self._state_lock:
            ...
            is_loading = self._loading.get(model_name, False)
            if model_name not in self._load_functions:
                logger.error(f"模型 {model_name} 未注册")
                return None
            load_func = self._load_functions[model_name]

        # 如果异步线程已经在加载同一模型，同步调用等待该线程，避免重复初始化。
        if is_loading and worker is not None:
            logger.info(f"等待正在异步加载的模型: {model_name}")
            worker.wait()
            with self._state_lock:
                if worker.result_model is not None:
                    self._models[model_name] = worker.result_model
                    self._loading[model_name] = False
                    return worker.result_model
                self._loading[model_name] = False
            if worker.error_message:
                logger.error(worker.error_message)
            return None
    ...
    def _cleanup_worker(self, model_name: str):
        """清理工作线程"""
        with self._state_lock:
            worker = self._workers.pop(model_name, None)
